/**
 * DropdownMenu hook — click-triggered floating menu with keyboard navigation.
 *
 * Combines Floating UI positioning with roving focus, typeahead, and menu
 * ARIA semantics. Designed to be reused by ContextMenu and Menubar.
 *
 ...
 *
 * Children found by:
 *   [data-dropdown-trigger] — the trigger element
 *   [data-dropdown-content] — the floating menu panel (role="menu")
 *
 * Sub-menus:
 *   [data-dropdown-sub]             — sub-menu wrapper (inside content)
 *   [data-dropdown-sub-trigger]     — item that opens a sub-menu
 *   [data-dropdown-sub-content]     — nested floating panel
 *
...
function getItems(container) {
  return Array.from(container.querySelectorAll(ITEM_SELECTOR)).filter((el) => {
    ...
    const subContent = el.closest("[data-dropdown-sub-content]");
...
function focusItem(item) {
  ...
  const menu =
    item.closest("[data-dropdown-sub-content]") ||
    item.closest("[data-dropdown-content]");
  if (menu) {
    menu
...
const DropdownMenu = {
  mounted() {
    ...
    // Find the actual button/anchor inside the trigger wrapper
    this._triggerButton =
      this._trigger.querySelector("button, a, [tabindex]") || this._trigger;
    ...
    this._onMenuKeydown = (e) => {
      ...
      switch (e.key) {
        ...
        case "ArrowRight": {
          ...
          if (activeEl && activeEl.hasAttribute("data-dropdown-sub-trigger")) {
            e.preventDefault();
            const sub = activeEl.closest("[data-dropdown-sub]");
        ...
        case " ": {
          ...
          if (activeEl && items.includes(activeEl)) {
            ...
            if (activeEl.hasAttribute("data-dropdown-sub-trigger")) {
              const sub = activeEl.closest("[data-dropdown-sub]");
              if (sub) this._showSub(sub);
            } else {
              activeEl.click();
    ...
    this._onContentClick = (e) => {
      ...
      if (item.hasAttribute("data-dropdown-sub-trigger")) {
        const sub = item.closest("[data-dropdown-sub]");
        if (sub) this._showSub(sub);
        return;
      }
    ...
    this._onContentMouseover = (e) => {
      const subTrigger = e.target.closest("[data-dropdown-sub-trigger]");
      if (subTrigger) {
        clearTimeout(this._subHoverTimer);
        const sub = subTrigger.closest("[data-dropdown-sub]");
        if (sub && (!this._openSub || this._openSub.sub !== sub)) {
          this._subHoverTimer = setTimeout(() => this._showSub(sub), 100);
        }
      } else if (
        e.target.closest("[data-dropdown-content]") === this._content &&
        !e.target.closest("[data-dropdown-sub-content]")
      ) {
  ...
  _showSub(sub) {
    ...
    const trigger = sub.querySelector("[data-dropdown-sub-trigger]");
    const content = sub.querySelector("[data-dropdown-sub-content]");
...
export { DropdownMenu as PhxShadcnDropdownMenu };
